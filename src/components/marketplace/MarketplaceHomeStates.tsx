...
interface MarketplaceHomeStatesProps {
...
export const MarketplaceHomeStates: React.FC<MarketplaceHomeStatesProps> = ({
  view,
  onDetectLocation,
  onRetry,
  onManualSubmit,
  ...
  if (view.status === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-white/70">
        <Loader2 className="h-8 w-8 animate-spin text-[#FF7A00]" />
        <p className="text-sm">Finding nearby kitchens…</p>
      </div>
    );
  }

  if (view.status === 'location_required' || view.status === 'location_denied' || view.status === 'location_unavailable') {
    const title =
    ...
    const description =
      view.status === 'location_denied'
        ? 'Enable location access or enter your address manually to see nearby kitchens.'
        : view.status === 'location_unavailable'
          ? view.error?.userMessage ?? 'We could not detect your location.'
          : 'Discover kitchens and restaurants near you.';
  ...
  if (view.status === 'empty') {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        <h2 className="text-lg font-semibold text-white">No kitchens nearby</h2>
        <p className="max-w-md text-sm text-white/60">
          We could not find kitchens within your delivery radius. Try a different location or check back later.
        </p>
        <button
          type="button"
          onClick={onRetry}
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        <h2 className="text-lg font-semibold text-white">
          {view.status === 'disabled' ? 'Marketplace unavailable' : 'Could not load kitchens'}
        </h2>
        <p className="max-w-md text-sm text-white/60">
...
export default MarketplaceHomeStates;
